...
- create_dim_aircraft_type — типы перелётов по количеству пересадок.
...
Для обеспечения корректной и надёжной последовательности выполнения этапов загрузки данных был разработан координирующий DAG dds_coordinator. 
Он реализует цепочку зависимостей, гарантируя, что каждый последующий этап запускается только после успешного завершения всех необходимых предшествующих операций.

DAG dds_coordinator начинает свою работу с сенсора ExternalTaskSensor, который отслеживает статус DAGа aviasales_cheap_tickets (загрузка сырых данных в ODS).
Для корректного сопоставления запусков используется кастомная функция execution_date_fn, которая динамически определяет execution_date последнего успешного запуска источника. 
Это позволяет избежать привязки к несуществующей или незавершённой дате выполнения.

После подтверждения готовности ODS, координатор инициирует цепочку трансформаций через операторы TriggerDagRunOperator:
- `dds_airline_scd2` создаёт или обновляет справочник авиакомпаний с поддержкой истории изменений (SCD Type 2). 
Этот этап выполняется первым, так как другие измерения и факт-таблица могут зависеть от актуального состояния справочника.
- `dds_dimensions` создаёт остальные измерения (dim_date, dim_route, dim_aircraft_type), обогащая их данными из ODS и обеспечивая нормализованную структуру.
- `dds_fact_flights` производит загрузку и обогащение факт-таблицы, выполняя сопоставление с уже созданными измерениями и обеспечивая ссылочную целостность.
- `data_quality_checks` запускается в самом конце и выполняет комплексную проверку качества данных в DDS-слое.

### Проверка качества данных (`data_quality_checks.py`)
После загрузки данных в слой DDS выполняется проверка качества через DAG data_quality_checks.
Проверяем
- объём данных
