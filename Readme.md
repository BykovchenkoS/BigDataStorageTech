...
| 2 | **Table** | `mart_airline_performance` | Позволяет сравнивать точные цифры: кол-во рейсов, средняя цена, доля прямых рейсов |
| 3 | **Pie Chart** | `mart_flight_type_analysis` | Визуализирует доли прямых, однопересадочных рейсов                |
...
- **Типы перелётов:**: 70% перелётов — прямые (direct), 30% — с одной остановкой (one_stop)→ Большинство пассажиров предпочитают прямые рейсы. 
...
- **Динамика цен по дням недели**: Самые дорогие билеты — в понедельник (12118 руб) и вторник (11187 руб), самые дешёвые — в субботу (7818 руб). → Выгоднее летать в выходные, дороже — в начале недели. Такие данные, наверное, из-за новогодних праздников. 31 декабря - это среда.
- **Авиакомпании**: Лидер по количеству рейсов: Pobeda (27 рейсов), но средняя цена у неё средняя (10779 руб). Самая высокая средняя цена: 3F (23762 руб), но всего 1 рейс. Наибольший % прямых рейсов: у нескольких перевозчиков 100%. → Pobeda доминирует по объёмам, но не по цене. - За анализируемый период выполнено 87 рейсов из Москвы
